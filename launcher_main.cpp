/* Multi-game launcher for GB-Recomp/pgbcomp.
 * Six Pokemon Gen 1 + Gen 2 carts in one binary, picked from a graphical
 * menu on launch (or via --game <id>). Each cart's recompiled C is pulled
 * in via FetchContent of the corresponding GB-Recomp/<id> repo. */
...
static int launch_pokeblue(int argc, char* argv[])    { return pokeblue_main(argc, argv); }
static int launch_pokeyellow(int argc, char* argv[])  { return pokeyellow_main(argc, argv); }
static int launch_pokegold(int argc, char* argv[])    { return pokegold_main(argc, argv); }
